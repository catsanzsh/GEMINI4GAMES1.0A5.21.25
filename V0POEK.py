...
# Game Boy sound features:
# - Pulse waves (Square waves with variable duty cycle: 12.5%, 25%, 50%, 75%) - This is now partially implemented!
# - Waveform channel (Arbitrary 32-sample, 4-bit waveform) - Not implemented here.
# - Noise channel (For percussion/SFX) - Not implemented here.
# - Sweep and Envelope units - Not implemented here.
# This engine focuses on variable duty cycle pulse waves for a more authentic Game Boy timbre.
...
def generate_square_wave_data(frequency, duration_ms, volume=0.1, duty_cycle=0.5):
    """
    Generates raw byte data for a square wave with variable duty cycle, meow! 
    Duty cycle (0.0 to 1.0) determines the pulse width. 
    Common Game Boy duty cycles: 0.125 (12.5%), 0.25 (25%), 0.5 (50%).
    It's so bouncy and full of energy, like a kitten on catnip!
    """
    if pygame.mixer is None: return bytearray() # No mixer, no soundy sound! That's a sad, silent world...
    if not (0 < duty_cycle <= 1.0):
        # print(f"WARNING: Duty cycle {duty_cycle} out of range (0, 1]. Defaulting to 0.5. Be a good kitty and use valid values!")
        duty_cycle = 0.5 # Default to 50% if out of sensible range, though GB only had specific ones.
    ...
    for i in range(num_samples):
        # This creates the square wave with variable duty cycle!
        # It's high for 'duty_cycle' proportion of the period, then low. So much zappy energy!
        if (i % period_samples) < (period_samples * duty_cycle):
            value = amplitude
        else:
            value = -amplitude
...
def create_tune_from_sequence(sequence, volume=0.05, default_duty_cycle=0.5):
    """
    Creates a pygame.mixer.Sound object from a sequence.
    Sequence items can be (note_name, duration_ms) or (note_name, duration_ms, duty_cycle).
    Nyah! It's a whole goddamn song, built from scratch! So creative!
    """
    if pygame.mixer is None: return None # Sad face, no music... This is the worst kind of silence.
    
    all_sound_data = bytearray()
    print("INFO: Generating tune sequence with duty cycles, this is gonna be fucking awesome, purr! Each note is a little piece of joy!")
    for item in sequence:
        note_name = item[0]
        duration_ms = item[1]
        duty = default_duty_cycle # Use the function's default duty cycle
        if len(item) == 3:
            duty = item[2] # Override with note-specific duty cycle if provided
        ...
        # print(f"DEBUG: Generating note: {note_name} ({freq} Hz) for {duration_ms}ms with duty {duty}. This is so exciting, like unwrapping a present!")
        note_data = generate_square_wave_data(freq, duration_ms, volume, duty_cycle=duty)
...
# This is our masterpiece, the legendary Pokémon Trainer Battle Theme (used for Rival Battles in Pokémon Red/Blue/Yellow),
# simplified for our awesome square wave synth, nya! Now with DUTY CYCLES for more authentic timbre!
# Sequence items: (Note Name, Duration in Milliseconds, Optional Duty Cycle - 0.125, 0.25, 0.5 are common GB values)
# It's gonna get everyone pumped the fuck up, meow! This tune is pure hype!
POKEMON_TRAINER_BATTLE_THEME = [
    # Phrase 1 (Iconic Intro/Main Riff part 1) - Using 50% and 25% duty cycles for variation
    ('FS5', 120, 0.5), ('PAUSE', 30), ('FS5', 120, 0.5), ('PAUSE', 30), ('FS5', 120, 0.25), ('E5', 120, 0.5), ('D5', 120, 0.5), 
    ('CS5', 200, 0.25), ('PAUSE', 40), ('CS5', 120, 0.5), ('B4', 120, 0.5),
    ('A4', 120, 0.25), ('B4', 120, 0.5), ('CS5', 240, 0.5), ('PAUSE', 120),

    # Phrase 2 (Main Riff part 2) - Keeps the energy flowing, purr!
    ('E5', 120, 0.5), ('PAUSE', 30), ('E5', 120, 0.5), ('PAUSE', 30), ('E5', 120, 0.25), ('D5', 120, 0.5), ('CS5', 120, 0.5),
    ('B4', 200, 0.25), ('PAUSE', 40), ('B4', 120, 0.5), ('A4', 120, 0.5), 
    ('GS4', 120, 0.25), ('A4', 120, 0.5), ('B4', 240, 0.5), ('PAUSE', 240), # Slightly longer pause to loop nicely, meow!
]
print("INFO: Pokémon Trainer Battle Theme sequence defined with DUTY CYCLES! This is gonna be legendary, purr! So many notes, so much excitement, so much TIMBRE!")
...
def draw_text_wrapped_pixel(surface, text, rect, color):
    ...
    # print(f"INFO: Drew wrapped text: '{text[:30]}...' which is super neat and fits perfectly, nya!") # This can be a bit spammy
...
if pygame.mixer: # Only try to make music if the mixer is happy, purr! Happy mixer, happy life!
    print("INFO: Attempting to create the fucking POKEMON BATTLE THEME with DUTY CYCLES, meow! This is gonna be epic!")
    # The default_duty_cycle in create_tune_from_sequence will be 0.5 if not specified per note.
    battle_music = create_tune_from_sequence(POKEMON_TRAINER_BATTLE_THEME, volume=0.06) 
    if battle_music:
...
while running:
    ...
    if not current_dialog_text: # If no dialog, let the game logic flow, like a beautiful river of code! It's so smooth and elegant!
        ...
        if battle_phase == "INTRO_1": display_dialog("RIVAL GARY WANTS TO FIGHT!")
            ...
            if selected_player_move["effect_type"] == "damage":
                ...
                # Display damage/miss message AFTER damage calculation and HP update
                if damage > 0: display_dialog(f"IT'S A HIT! SO MUCH POWER, MEOW!") 
                else: display_dialog(f"{player_pokemon.name.upper()}'S ATTACK MISSED OR HAD NO EFFECT! WOAH!")
                battle_phase = "PLAYER_DAMAGE_MSG" # Transition after message is set
    ...
    if current_dialog_text or battle_phase == "PLAYER_PROMPT": # Dialog box time, let's tell a story! Every word is part of the grand adventure!
        ...
        if current_dialog_text: # Only draw if there's actually text, makes sense, right? So logical!
            ...
            # Only show advance indicator if it's not a battle end state or a choice prompt without dialog
            can_advance_dialog = not (battle_phase.startswith("BATTLE_END") or \
                                 (battle_phase == "ACTION_SELECT" and not current_dialog_text) or \
                                 (battle_phase == "MOVE_SELECT" and not current_dialog_text) or \
                                 (battle_phase == "PLAYER_PROMPT" and current_dialog_text.startswith("WHAT WILL")))


            if can_advance_dialog and pygame.time.get_ticks() // 400 % 2 == 0: # Blinky indicator, so professional and user-friendly!
